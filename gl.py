...
from MathLib import normalizarVector
import pygame
import random
...
class RendererRT(object):
    ...
    def glPoint(self, x, y, color=None):
        # Pygame empieza a renderizar desde la esquina superior izquierda. Hay que voltear el valor y
        x = round(x)
        y = round(y)
    ...
    def glRender(self):
        ...
        for i,j in indexes:
                ...
                if 0 <= x < self.width and 0 <= y < self.height:
                    ...
                    # Escalando a las dimensiones de la cámara
                    pX *= self.rightEdge
                    pY *= self.topEdge
                    ...
                    # Normalizamos el vector dir usando las funciones creadas
                    dir = normalizarVector(dir)

                    intercept = self.glCastRay(self.camera.translate, dir)
                    
                    color = [0,0,0]
                    # Verificamos si el rayo da en algún objeto
                    if intercept != None:
